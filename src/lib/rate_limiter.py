...
class RateLimiter:
    ...
    def __init__(self, calls_per_minute: int = 60, burst_size: int | None = None):
    ...
    async def acquire(self, tokens: int = 1) -> bool:
        ...
        async with self.lock:
            ...
            # Rate limited
            logger.warning(f"Rate limit reached: {self.tokens}/{self.burst_size} tokens available")
            return False
    ...
    def get_stats(self) -> dict[str, any]:
        ...
        recent_calls = sum(1 for call_time in self.call_history if now - call_time < 60)
...
class MultiServiceRateLimiter:
    ...
    def add_service(
        ...
        logger.info(f"Added rate limiter for {service_name}: {calls_per_minute} calls/min")
