...
class CLI:
    ...
    def __init__(self, debug: bool = False, show_reflection: bool = False):
        ...
        self.conversation: Optional[ConversationSession] = None
            ...
            try:
                ...
                try:
                    episode_record = self.memory_vault.add_episode(
                        ...
                        confidence=min(max(response.confidence, 0.0), 1.0) if hasattr(response, 'confidence') else None,
                ...
                if self.reflection_agent and episode_record:
                    try:
                        ...
                        tools_used = [t.tool_name for t in response.tool_results] if response.tool_results else []
    ...
    def _show_cost(self):
        ...
        if cost_summary['soft_cap_reached']:
            print(f"   Status: ⚠️  Soft cap reached - using cheaper models")
        elif cost_summary['hard_cap_reached']:
            print(f"   Status: 🛑 Hard cap reached - local models only")
        else:
    ...
    def _handle_memory_command(self, cmd: str):
        ...
        if action == "list":
            ...
            for r in recs:
                print(f"  - [{r.get('type')}] {r.get('id')} | {r.get('created_at')} | {r.get('summary') or '(no summary)'}")
            print()
            return
        if action == "export":
        ...
        if action == "prune":
            ...
            print(f"\n🧹 Pruned {total} memories: " + ", ".join([f"{k}={v}" for k, v in stats.items() if v]))
            print()
            return
