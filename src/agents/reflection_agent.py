...
class ReflectionAgent:
    ...
    async def distillation_sweep(
        ...
        try:
            ...
            episodes = [
                e for e in self.vault.list(mtype="episodic", limit=100)
                if datetime.fromisoformat(e["created_at"]) > cutoff
            ]
            
            reflections = [
                r for r in self.vault.list(mtype="reflection", limit=100)
            ...
            if distilled.get("prompts"):
                for prompt_item in distilled["prompts"][:3]:
                    ...
                    if isinstance(prompt_item, dict):
                        prompt_text = prompt_item.get("prompt", prompt_item.get("pattern", str(prompt_item)))
                    else:
            ...
            if distilled.get("procedures"):
                for proc_item in distilled["procedures"][:3]:
                    ...
                    if isinstance(proc_item, dict):
                        proc_text = proc_item.get("procedure", proc_item.get("step", str(proc_item)))
                    else:
