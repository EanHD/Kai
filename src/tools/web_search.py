...
class WebSearchTool(BaseTool):
    ...
    def _enhance_query(self, query: str) -> str:
        ...
        time_words = [
            "latest", "new", "upcoming", "recent", "today", "now",
            "concert", "event", "show", "festival", "happening", "current"
        ]
        ...
        has_date = any(month in query_lower for month in [
    ...
    def _filter_by_date(self, results: list[dict], max_days_old: int) -> list[dict]:
        ...
        logger.debug(f"Date filter: {len(results)} → {len(filtered)} results (max {max_days_old} days)")
        return filtered
