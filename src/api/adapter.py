...
class OrchestratorAdapter:
    ...
    def convert_request(
        ...
        orchestrator_request = {k: v for k, v in orchestrator_request.items() if v is not None}
    ...
    def convert_response(
        ...
        if "tool_calls" in orchestrator_response:
            openai_response["choices"][0]["message"]["tool_calls"] = orchestrator_response[
                "tool_calls"
            ]
    ...
    async def invoke_orchestrator(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        logger.info(
            f"Invoking orchestrator: provider={request.get('provider')}, model={request.get('model')}"
        )
        ...
        if self.reflection_agent and self.memory_vault:
            ...
            asyncio.create_task(
                self._reflect_on_api_interaction(
                    query_text=query_text,
        ...
        return {
            ...
            else request.get("model", "unknown"),
        }
    ...
    async def invoke_orchestrator_stream(
        ...
        """
        logger.info(
            f"Invoking orchestrator stream: provider={request.get('provider')}, model={request.get('model')}"
        )
    ...
    async def _reflect_on_api_interaction(
        ...
        try:
            ...
            if episode_record:
                ...
                logger.debug("Background reflection completed for API interaction")
        except Exception as e:
